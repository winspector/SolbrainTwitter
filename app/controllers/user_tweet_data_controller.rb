...
class UserTweetDataController < ApplicationController
  ...
  def index
    ...
    @friend_of_acquire_users = TwitterConnection.get_friend_of_acquire_users_limit(acquire_uid[:account_uid], 200, 5)
  ...
  def show_tweet_of_acquire_origin
    ...
    target_acquire_friends = TwitterConnection.get_friend_of_acquire_users_limit(acquire_uid, 200, 5)
    puts "target_acquire_friends:#{target_acquire_friends.count}"
    UserTweetData.exec_get_acquire_friends_tweet_data(target_user, params[:screen_name], target_acquire_friends)

    # 起点ユーザのツイート表示画面に遷移する
    redirect_to '/user_tweet_data/index'
  end
end